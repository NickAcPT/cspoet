...
public class FileReadingTest {
  ...
  @Test public void javaFileObjectUri() {
    ...
    assertThat(CSharpFile.builder("", type).build().toJavaFileObject().toUri())
        .isEqualTo(URI.create("Test.java"));
    assertThat(CSharpFile.builder("foo", type).build().toJavaFileObject().toUri())
        .isEqualTo(URI.create("foo/Test.java"));
    assertThat(CSharpFile.builder("com.example", type).build().toJavaFileObject().toUri())
        .isEqualTo(URI.create("com/example/Test.java"));
  }
  
  @Test public void javaFileObjectKind() {
    CSharpFile csharpFile = CSharpFile.builder("", TypeSpec.classBuilder("Test").build()).build();
    assertThat(csharpFile.toJavaFileObject().getKind()).isEqualTo(Kind.SOURCE);
  }
  ...
  @Test public void javaFileObjectCharacterContent() throws IOException {
    TypeSpec type = TypeSpec.classBuilder("Test")
        ...
        .build();
    CSharpFile csharpFile = CSharpFile.builder("foo", type).build();
    JavaFileObject javaFileObject = csharpFile.toJavaFileObject();
    
    // We can never have encoding issues (everything is in process)
    assertThat(javaFileObject.getCharContent(true)).isEqualTo(csharpFile.toString());
    assertThat(javaFileObject.getCharContent(false)).isEqualTo(csharpFile.toString());
  }
  
  @Test public void javaFileObjectInputStreamIsUtf8() throws IOException {
    CSharpFile csharpFile = CSharpFile.builder("foo", TypeSpec.classBuilder("Test").build())
        .addFileComment("Pi\u00f1ata\u00a1")
        .build();
    byte[] bytes = ByteStreams.toByteArray(csharpFile.toJavaFileObject().openInputStream());
    
    // JavaPoet always uses UTF-8.
    assertThat(bytes).isEqualTo(csharpFile.toString().getBytes(UTF_8));
  }
  ...
  @Test public void compileJavaFile() throws Exception {
    ...
    CompilationTask task = compiler.getTask(null, 
        ...
        Collections.emptySet(),
        Collections.singleton(csharpFile.toJavaFileObject()));
