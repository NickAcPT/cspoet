...
public final class FileWritingTest {
  ...
  @Test public void pathNotDirectory() throws IOException {
    ...
    CSharpFile csharpFile = CSharpFile.builder("example", type).build();
    Path path = fs.getPath("/foo/bar");
    Files.createDirectories(path.getParent());
    Files.createFile(path);
    try {
      csharpFile.writeTo(path);
      fail();
  ...
  @Test public void pathDefaultPackage() throws IOException {
    TypeSpec type = TypeSpec.classBuilder("Test").build();
    CSharpFile.builder("", type).build().writeTo(fsRoot);
  ...
  @Test public void fileDefaultPackage() throws IOException {
    TypeSpec type = TypeSpec.classBuilder("Test").build();
    CSharpFile.builder("", type).build().writeTo(tmp.getRoot());
  ...
  @Test public void filerDefaultPackage() throws IOException {
    TypeSpec type = TypeSpec.classBuilder("Test").build();
    CSharpFile.builder("", type).build().writeTo(filer);
  ...
  @Test public void pathNestedClasses() throws IOException {
    TypeSpec type = TypeSpec.classBuilder("Test").build();
    CSharpFile.builder("foo", type).build().writeTo(fsRoot);
    CSharpFile.builder("foo.bar", type).build().writeTo(fsRoot);
    CSharpFile.builder("foo.bar.baz", type).build().writeTo(fsRoot);
  ...
  @Test public void fileNestedClasses() throws IOException {
    TypeSpec type = TypeSpec.classBuilder("Test").build();
    CSharpFile.builder("foo", type).build().writeTo(tmp.getRoot());
    CSharpFile.builder("foo.bar", type).build().writeTo(tmp.getRoot());
    CSharpFile.builder("foo.bar.baz", type).build().writeTo(tmp.getRoot());
  ...
  @Test public void filerNestedClasses() throws IOException {
    TypeSpec type = TypeSpec.classBuilder("Test").build();
    CSharpFile.builder("foo", type).build().writeTo(filer);
    CSharpFile.builder("foo.bar", type).build().writeTo(filer);
    CSharpFile.builder("foo.bar.baz", type).build().writeTo(filer);
  ...
  @Test public void filerPassesOriginatingElements() throws IOException {
    ...
    CSharpFile.builder("example", test1).build().writeTo(filer);
    CSharpFile.builder("example", test2).build().writeTo(filer);
  ...
  @Test public void filerClassesWithTabIndent() throws IOException {
    TypeSpec test = TypeSpec.classBuilder("Test")
        ...
        .addMethod(MethodSpec.methodBuilder("main")
            ...
            .build())
        .build();
    CSharpFile.builder("foo", test).indent("\t").build().writeTo(filer);
  ...
  @Test public void fileIsUtf8() throws IOException {
    CSharpFile csharpFile = CSharpFile.builder("foo", TypeSpec.classBuilder("Taco").build())
        .addFileComment("Pi\u00f1ata\u00a1")
        .build();
    csharpFile.writeTo(fsRoot);
